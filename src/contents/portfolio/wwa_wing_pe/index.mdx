...
中止から約5年後の2023年から開発を再開。なるべく前述のリファクタリングと干渉しない形で WWA Wing に簡易的にピクチャを表示する追加機能を構想していましたが、期待する声も大きく、結果的にはアニメーション機能も含めた、 WWA Wing PE と同等の実装を目指すことになりました。なお、5年経過したことで TypeScript のモジュールシステムや WWA Wing の基礎部分も変わったことから、 WWA Wing PE のソースコードをそのまま流用することはなく、1から実装することになりました。

WWA Wing の開発で説明されている「全く新しいゲームシステムの開発」のところでは、実装したピクチャ機能で必要なプロパティを集計して、さらに機能強化する予定でいます。
